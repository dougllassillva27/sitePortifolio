...
create index if not exists agendamentos_barbeiro_periodo_idx
...
select 'Douglas Silva', 'Doug', '/assets/img/favicon.svg', 'Especialista em cortes clássicos, degradê limpo e acabamento detalhado.', 'primary'
where not exists (select 1 from barbeiros where apelido = 'Doug');

insert into barbeiros (nome, apelido, foto_url, descricao, google_calendar_id)
select 'Marcos Oliveira', 'Marcos', '/assets/img/favicon.svg', 'Foco em barba, corte social e atendimento rápido para rotina corrida.', 'primary'
where not exists (select 1 from barbeiros where apelido = 'Marcos');

insert into servicos (nome, descricao, preco_centavos, duracao_minutos)
select 'Corte', 'Corte masculino com acabamento na navalha.', 4500, 30
where not exists (select 1 from servicos where nome = 'Corte');

insert into servicos (nome, descricao, preco_centavos, duracao_minutos)
select 'Barba', 'Barba desenhada com toalha quente e finalização.', 3500, 20
where not exists (select 1 from servicos where nome = 'Barba');

insert into servicos (nome, descricao, preco_centavos, duracao_minutos)
select 'Corte + Barba', 'Combo completo para sair pronto.', 7500, 50
where not exists (select 1 from servicos where nome = 'Corte + Barba');

insert into servicos (nome, descricao, preco_centavos, duracao_minutos)
select 'Sobrancelha', 'Limpeza e alinhamento rápido.', 2000, 15
